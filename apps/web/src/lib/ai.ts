...
以下のJSON形式で出力してください（他のテキスト不要）：
{
  ...
  "narrationText": "（音声読み上げ用テキスト。記事全体を1つの連続したナレーションとして構成する。【重要：数字の読み方】音声合成が正しく読めるよう、数字はすべて日本語の読みで書くこと。例: 2.0→にーてんぜろ、V8→ブイはち、4WD→よんダブリューディー、300ps→さんびゃくピーエス、6速→ろくそく、1960年→せんきゅうひゃくろくじゅうねん、110→ひゃくじゅう。アルファベット略語もカタカナで書く（AMG→エーエムジー、GT→ジーティー）。以下のElevenLabs v3オーディオタグを適切に挿入すること：[calm]落ち着いたトーン（本文ベース）、[whispers]囁き（五感の繊細な描写）、[confident]自信のあるトーン（プルクォート・決め台詞）、[slow pace]ゆっくり（冒頭・重要フレーズ）、[pause Xs]間（セクション間に1〜2秒）。冒頭は[deep, authoritative]＋[pause 1.5s]で車名から始め、セクション間に[pause 1.5s]を入れ、プルクォートは[confident]で読む。1500文字程度）"
}
`
