...
以下のJSON形式で出力してください（他のテキスト不要）：
{
  ...
  "narrationText": "（音声読み上げ用テキスト。【絶対遵守】文字数は最低1500文字、最大1800文字（タグ・空白含む）。1500文字未満は失敗とみなす。記事全体（section1+2+3+pullQuote+締めエピソード）を1つの連続したナレーションとして再構成し、十分なボリュームで書ききること。【重要：数字の読み方】音声合成が正しく読めるよう、数字はすべて日本語の読みで書くこと。数字はカタカナで音を書くこと。例: 2.0→ニーテンゼロ、V8→ブイハチ、4WD→ヨンダブリューディー、300ps→サンビャクピーエス、6速→ロクソク、1960年→センキュウヒャクロクジュウネン、110→ヒャクジュウ。アルファベット略語もカタカナで書く（AMG→エーエムジー、GT→ジーティー）。固有名詞の読み：HI-TOP→ハイトップ、HI-TOP JOURNAL→ハイトップジャーナル、HI-TOP CORPORATION→ハイトップコーポレーション。以下のElevenLabs v3オーディオタグを適切に挿入すること：[calm]落ち着いたトーン（本文ベース）、[whispers]囁き（五感の繊細な描写）、[confident]自信のあるトーン（プルクォート・決め台詞）、[slow pace]ゆっくり（冒頭・重要フレーズ）、[pause Xs]間（セクション間に1〜2秒）。冒頭は[deep, authoritative]＋[pause 1.5s]で車名から始め、セクション間に[pause 1.5s]を入れ、プルクォートは[confident]で読む。【構成・文字数の目安】①冒頭の車名導入: 50字前後 → ②section1の物語展開（モデル固有の歴史・豆知識・五感描写）: 350-450字 → ③section2の物語展開（走り・スペック・カスタム）: 350-450字 → ④section3のディテール語り: 200-300字 → ⑤pullQuote 響き直し: 50-100字 → ⑥締めエピソード（情景・人生のワンシーン）: 300-400字 → ⑦ブランドメッセージ: 100字前後。合計1500-1800字。【締めくくり】ナレーション終盤は2段構成にする。①[calm]または[whispers]で、この車にまつわるジーンとくる短いエピソードや情景描写を入れる（3〜5文）。その車が誰かの人生の一場面にどう寄り添うか、オーナーになった未来の一瞬、あるいはこの車が歩んできた時間への敬意など。聴いている人の心に残る小話にすること。毎回まったく異なるストーリーで。②その後[pause 1.5s]を挟み、[confident]でハイトップのブランドメッセージで締める。毎回異なる表現で、以下を自然に織り交ぜること：会社名『ハイトップ』『ハイトップコーポレーション』、スローガン『マイ パッション イズ ワン コード』。定型文にせず、車や記事の内容に合わせて毎回変えること。【再確認】出力前に narrationText の文字数を数え、1500未満なら追加描写を入れて1500を超えるまで書くこと。）"
}
`.trim()
}
